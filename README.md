...
* The structure of this repository is as follows:
  ...
  * [`config.py`](config.py) contains the settings for a particular training run. Note that settings specific to the datasets and dataloader are specified in [`settings.py`](/datasets/settings.py) from the [`datasets/Generic_ViCCT`](/datasets/Generic_ViCCT) folder.
